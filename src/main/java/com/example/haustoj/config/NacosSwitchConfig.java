...
public class NacosSwitchConfig {
    ...
    public void init() throws NacosException {
        if (init.compareAndSet(false, true)) {
            ...
            properties.setProperty(PropertyKeyConst.SERVER_ADDR, NACOS_URL);
            ...
            configService.addListener(switchConfigFileName, group, new Listener() {
                ...
                public void receiveConfigInfo(String configInfo) {
                    log.info("[Switch Config] [Receive] [{}]", configInfo);
    ...
    public boolean publishSwitchConfig(SwitchConfig switchConfig) {
        DumperOptions options = new DumperOptions();
        options.setDefaultFlowStyle(DumperOptions.FlowStyle.BLOCK);
        options.setDefaultScalarStyle(DumperOptions.ScalarStyle.PLAIN);
        Yaml yaml = new Yaml(options);
    ...
    public boolean publishWebConfig(WebConfig webConfig) {
        DumperOptions options = new DumperOptions();
        options.setDefaultFlowStyle(DumperOptions.FlowStyle.BLOCK);
        options.setDefaultScalarStyle(DumperOptions.ScalarStyle.PLAIN);
        Yaml yaml = new Yaml(options);
        String content = yaml.dumpAsMap(webConfig);
        try {
            return configService.publishConfig(webConfigFileName, group, content, ConfigType.YAML.getType());
        } catch (NacosException e) {
            log.error("[Nacos Config] publish web config error:{}, config:{}", e, content);
